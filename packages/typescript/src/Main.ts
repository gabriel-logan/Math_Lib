 ...
 */
export default class Calculator extends ArithmeticOp {
	...
	private generateCriticalPointInterval(min: number, max: number) {
	...
	protected newtonMethod(
		...
		if (answer1 > answer2) {
			if (Number(answer1) - Number(answer2) > 1) {
				criticalPoint3 = this.generateCriticalPointInterval(
	...
	protected ruffiniDevice(
		...
		const second = secondCoefficient * roots[0];

		const thirdCoefficient = Number(second) + Number(valueC);

		const third = thirdCoefficient * roots[0];
		...
		if (fourthCoefficient == 0) {
			...
			if (delta < 0) {
				return {
					value: [0],
					msg: `It has only 1 real root in X = ${roots[0]}`,
				};
			} else {
